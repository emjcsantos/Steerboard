...
describe("project management hierarchy", () => {
  ...
  it("keeps current Phase 3 and Phase 11 PM descriptions aligned to clearance snapshot and proof depth gates", () => {
    ...
    expect(byId.get("phase-11-parent-release-packaging")?.description).toContain("current Phase 3 clearance PM traceability and handoff proof");
    expect(byId.get("phase-11-child-package-validation")?.description).toContain("current Phase 3 clearance PM traceability and handoff proof");
    expect(byId.get("phase-11-child-traceability")?.description).toContain("proof freshness depth");
    expect(byId.get("phase-11-child-traceability")?.description).toContain("current Phase 3 clearance PM traceability and handoff proof");
  });
