...
export const steerboardMilestoneStatuses: MilestoneStatus[] = [
  ...
  },
  {
    ...
    nextStep:
      "Run the full local testing checklist, keep `npm.cmd run smoke:phase3` as the recurring opt-in desktop smoke command, run `npm.cmd run test:phase3:owner-visible` for the non-live imported-proof panel check, run `npm.cmd run test:phase11:owner-visible` for the non-live Owner Command, Proof Freshness, Evidence Records, and Release Readiness proof, use the Phase 3 gate actions to clear missing proofs, verify smoke proof rows move browser or non-executed proofs to waiting and desktop executions to ready, review, or blocked across reload and while the app remains open, verify the per-control Owner Testing state chips, approval-to-probe behavior, all-catalog provider refresh smoke, live panel slash execution evidence, live panel control evidence, active-turn interrupt/steer smoke proof, Phase 11 owner command evidence, and disabled-path evidence (terminal, Git, MCP, plugin, automation, runtime, profile, and external), and keep desktop-backed mutation as a future phase.",
    tone: "active",
  ...
  },
  {
    ...
    latestNote:
      "The Phase 11 Release Readiness gate now tracks clean checkout, build/test, smoke proof, current active Phase 3 clearance PM traceability with handoff proof, packaging lock, docs/known limits, final security closure capability, and structured release-decision evidence while packaging remains paused.",
    nextStep:
      "Use the Phase 11 Release Readiness panel to record the final release proof after live workflow, provider, migration, audit, Phase 3 traceability, final security closure, owner proof, and release-decision evidence holds are clear; do not package, sign, create installers, or push from this gate.",
    tone: "paused",
