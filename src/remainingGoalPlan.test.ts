...
describe("remaining goal plan", () => {
  ...
  it("summarizes the current remaining goal queue", () => {
    expect(summarizeRemainingGoalPlan()).toEqual({
      ...
      currentNextAction:
        "Use the Phase 4 Provider Readiness catalog depth, structured catalog-depth aggregate proof, structured command/skill aggregate proof, command/skill catalog item-order, evidence-key, scoped command, skill invocation, and execution-lock proof, structured plugin/MCP aggregate proof, plugin/MCP metadata-only surface, scoped plugin surface, scoped MCP transport/tool-policy, and execution-lock proof, owner-visible plugin/MCP metadata-only safety evidence, Refresh Safety depth, structured all-catalog refresh-smoke proof, structured refresh-safety depth aggregate proof, reload-safe recorded metadata-only proof, Surface Depth owner-boundary proof, structured surface-depth aggregate proof, local approval, structured audit-chain, structured rollback-chain, structured permission-chain, and permission record validation, owner-visible rollback and permission record-chain evidence, visible record-enable gates, explicit record-chain traceability row, tested recorded provider-review artifact loading with offline fingerprint verification, attached local record evidence, missing-record review enforcement, owner-visible missing-record proof, catalog-depth aggregate artifact enforcement, command/skill aggregate artifact enforcement, plugin/MCP aggregate artifact enforcement, refresh-smoke artifact enforcement, refresh-safety depth aggregate artifact enforcement, audit validation chain-proof artifact enforcement, rollback validation chain-proof artifact enforcement, permission validation chain-proof artifact enforcement, surface-depth aggregate artifact enforcement, surface owner-boundary artifact enforcement, compact top-blocker source/evidence/status proof, traceability, blocker-priority panels, and owner-visible provider readiness check to keep source coverage, setup blockers, capability gaps, preview rows, approval, audit, rollback, permission, fresh fingerprint-matched metadata-only refresh proof, PM links, and execution locks ready before provider execution is considered.",
  ...
  it("keeps the Phase 4 provider surfaces target linked to traceability and surface depth", () => {
    ...
    expect(phase4Epic?.completionPercent).toBe(91);
    ...
    expect(phase4Goal?.nextAction).toContain("explicit record-chain traceability row");
    expect(phase4Goal?.nextAction).toContain("reload-safe recorded metadata-only proof");
    expect(phase4Goal?.nextAction).toContain("structured surface-depth aggregate proof");
    expect(phase4Goal?.nextAction).toContain(
      "local approval, structured audit-chain, structured rollback-chain, structured permission-chain, and permission record validation"
    );
    ...
    expect(phase4Goal?.nextAction).toContain("surface-depth aggregate artifact enforcement");
    ...
    expect(phase4Goal?.nextAction).toContain("command/skill aggregate artifact enforcement");
    expect(phase4Goal?.nextAction).toContain("plugin/MCP aggregate artifact enforcement");
    expect(phase4Goal?.nextAction).toContain("compact top-blocker source/evidence/status proof");
