...
describe("remaining goal plan", () => {
  ...
  it("keeps the Phase 4 provider surfaces target linked to traceability and surface depth", () => {
    ...
    expect(phase4Epic?.completionPercent).toBe(94);
    ...
    expect(phase4Goal?.nextAction).toContain(
      "local approval, structured approval-chain, structured audit-chain with approval-validation linkage, structured rollback-chain with audit-validation linkage, structured permission-chain, and permission record validation"
    );
