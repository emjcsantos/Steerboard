...
describe("phase 4 provider review artifact", () => {
  ...
  it("reviews no-blocker artifacts with incomplete refresh-safety depth aggregate proof", () => {
    const artifact = withReadyLocalRecords(
      reviewArtifact({
        refreshSmoke: liveCatalogRefreshSmoke()
      })
    );
    const incompleteRefreshSafetyDepthProof: Phase4ProviderReviewArtifact = {
      ...artifact,
      refreshSafety: {
        ...artifact.refreshSafety,
        refreshSafetyDepthProof: artifact.refreshSafety.refreshSafetyDepthProof.replace(
    ...
    expect(
      verifyPhase4ProviderReviewArtifact(incompleteRefreshSafetyDepthProof, {
        verifiedAt: "2026-06-18T10:05:00.000Z",
        expectedCatalogFingerprint: artifact.currentCatalogFingerprint
      })
    ).toMatchObject({
  ...
  it("reviews no-blocker artifacts with incomplete rollback audit-chain linkage proof", () => {
    const artifact = withReadyLocalRecords(
      reviewArtifact({
        refreshSmoke: liveCatalogRefreshSmoke()
      })
    );
    const withoutRollbackAuditLink: Phase4ProviderReviewArtifact = {
      ...artifact,
      rollbackValidation: artifact.rollbackValidation
        ? {
            ...artifact.rollbackValidation,
            rollbackChainProof: artifact.rollbackValidation.rollbackChainProof.replace(
    ...
    expect(
      verifyPhase4ProviderReviewArtifact(withoutRollbackAuditLink, {
        verifiedAt: "2026-06-18T10:05:00.000Z",
        expectedCatalogFingerprint: artifact.currentCatalogFingerprint
      })
    ).toMatchObject({
  ...
  it("reviews no-blocker artifacts with incomplete permission rollback-chain linkage proof", () => {
    const artifact = withReadyLocalRecords(
      reviewArtifact({
        refreshSmoke: liveCatalogRefreshSmoke()
      })
    );
    const withoutPermissionRollbackLink: Phase4ProviderReviewArtifact = {
      ...artifact,
      permissionValidation: artifact.permissionValidation
        ? {
            ...artifact.permissionValidation,
            permissionChainProof: artifact.permissionValidation.permissionChainProof.replace(
    ...
    expect(
      verifyPhase4ProviderReviewArtifact(withoutPermissionRollbackLink, {
        verifiedAt: "2026-06-18T10:05:00.000Z",
        expectedCatalogFingerprint: artifact.currentCatalogFingerprint
      })
    ).toMatchObject({
