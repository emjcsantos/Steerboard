...
6. `Slash Commands`
   ...
   - After manually running `npm.cmd run smoke:phase3`, use the Phase 3 command validation record to capture the local CLI pass, then confirm this freshness-reviewed record does not mark persisted desktop UI proof rows ready or unlock handoff by itself.
   ...
   - Reload after a full desktop smoke bundle and confirm the Phase 3 smoke proof rows keep the last desktop-executed proof states without rerunning prompts on startup, while stale `checkedAt` values downgrade to review after reload and during long-open app sessions.
