...
7. `Catalogs`
   ...
   - Confirm the Phase 4 provider review artifact stays metadata-only, includes catalog-depth, refresh-safety, surface-depth, traceability, blocker-priority, execution-lock, current/expected catalog-fingerprint match state, and local record-validation evidence, and returns to review when its six-surface catalog fingerprint or attached approval/audit/rollback/permission validation no longer matches the current provider state.
