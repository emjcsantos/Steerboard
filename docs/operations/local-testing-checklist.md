...
6. `Slash Commands`
   ...
   - Confirm focusing an Arena panel makes Phase 3 slash/session readiness use that panel's evidence before any other panel's ready proof, and that a focused panel with no proof stays waiting instead of borrowing another panel's proof.
   ...
   - Confirm the Owner Testing Phase 3 handoff gate holds provider integration until desktop proof clearance is ready, exact blocker visibility is clear, current active goal and required PM traceability are trusted, and a non-expired, non-future-dated owner handoff record matching the compact current evidence fingerprint is attached with age-checked validation.
