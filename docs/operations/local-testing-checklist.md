...
2. `Connect`
   ...
   - Confirm `Record approval` writes only a local Phase 4 provider approval record after refresh-safety proof is ready, and that the record stays trusted only while it is fresh and matches the current six-surface catalog fingerprint.
   - Confirm `Record audit` writes only a local Phase 4 provider audit record after approval is ready, and that the record stays trusted only while it is fresh, approval-matched, catalog-matched, audit-fingerprint-matched, and mutation-locked.
   - Confirm `Record rollback` writes only a local Phase 4 provider rollback record after audit is ready, and that the record stays trusted only while it is fresh, approval-matched, audit-record-matched, audit-evidence-matched, catalog-matched, surface-depth-fingerprint-matched, rollback owner/action-present, and mutation-locked.
   - Confirm `Record permission` writes only a local Phase 4 provider permission record after rollback is ready, and that the record stays trusted only while it is fresh, approval-matched, audit-record-matched, rollback-record-matched, audit-evidence-matched, rollback-evidence-matched, catalog-matched, surface-depth-fingerprint-matched, permission-fingerprint-matched, all-six-surface-scoped, owner/scope/action-present, and mutation-locked.
   ...
   - Optionally run `npm.cmd run smoke:phase3:record` to execute the same Phase 3 smoke command and write `local_private/phase3-command-validation-record.json` plus `local_private/phase3-smoke-proof-bundle.json` for Owner Testing review. Run `npm.cmd run smoke:phase3:verify` before owner review to confirm both recorded artifacts are present, passed, and backed by three desktop-executed proof rows. In desktop mode, use the Owner Testing `Load recorded` buttons to read those known local artifacts directly; in browser preview, use the file import controls. The command-validation artifact records CLI validation only; the smoke-proof bundle carries the desktop-executed proof rows that still pass the per-row storage filter.
3. `Chat`
...
6. `Slash Commands`
   ...
   - After `npm.cmd run smoke:phase3:record`, load the recorded `local_private/phase3-smoke-proof-bundle.json` from desktop mode or import it from browser preview through the Phase 3 desktop smoke readiness rows, then confirm only desktop-executed rows are preserved in persisted proof storage with per-row storage proof source, createdAt timestamp, and matching proof fingerprint.
...
7. `Catalogs`
   ...
   - Confirm the Phase 4 approval record controls are visible, `Record approval` stores only local metadata tied to the current six-surface catalog fingerprint and fresh refresh-safety proof, `Clear approval` removes that local record, and stale, malformed, future-dated, or fingerprint-mismatched records return the approval gate to review.
   - Confirm the Phase 4 audit record controls are visible, `Record audit` stores only local metadata tied to the current approval record, six-surface catalog fingerprint, and current audit evidence fingerprint, `Clear audit` removes that local record, and stale, malformed, future-dated, approval-mismatched, catalog-mismatched, audit-fingerprint-mismatched, or mutation-unlocked records return the audit gate to review.
   - Confirm the Phase 4 rollback record controls are visible, `Record rollback` stores only local metadata tied to the current approval record, audit record, audit evidence fingerprint, six-surface catalog fingerprint, and current surface-depth rollback evidence fingerprint, `Clear rollback` removes that local record, and stale, malformed, future-dated, approval-mismatched, audit-mismatched, audit-evidence-mismatched, catalog-mismatched, surface-depth-fingerprint-mismatched, missing owner/action, or mutation-unlocked records return the rollback gate to review.
   - Confirm the Phase 4 permission record controls are visible, `Record permission` stores only local metadata tied to the current approval record, audit record, rollback record, audit evidence fingerprint, rollback evidence fingerprint, six-surface catalog fingerprint, current surface-depth evidence fingerprint, current permission evidence fingerprint, and all six provider surface scopes, `Clear permission` removes that local record, and stale, malformed, future-dated, approval-mismatched, audit-mismatched, rollback-mismatched, audit-evidence-mismatched, rollback-evidence-mismatched, catalog-mismatched, surface-depth-fingerprint-mismatched, permission-fingerprint-mismatched, missing surface scope, missing owner/scope/action, or mutation-unlocked records return the permission gate to review.
   - Run `npm.cmd run test:phase4:owner-visible` to verify the Phase 4 Provider Readiness and Surface Depth panels render catalog source labels, totals, evidence expectations, local approval/audit/rollback/permission record controls, evidence keys, execution-lock guidance, and surface next actions as visible owner-review text without running provider actions.
   ...
   - Confirm the Phase 4 Provider Traceability rows link `goal-phase-4-provider-surfaces`, all required Phase 4 PM child rows including `phase-04-child-blocker-priority`, catalog-depth evidence, fresh fingerprint-matched refresh-safety evidence, surface-depth evidence, provider execution locks, and current-active Phase 4 goal state, block when a required row is missing from either the board plan or the remaining-goal links, and keep Phase 4 provider review held with the current active goal id visible until Phase 4 becomes the single current active remaining goal.
   - Confirm the Phase 4 Provider Blocker Priority rows rank the exact top provider blocker, show whether catalog smoke can address it, keep remaining-goal or PM traceability blockers out of catalog-smoke actions, and keep setup-required or unavailable provider blockers ahead of preview-only refresh proof.
   - Confirm the Phase 4 provider review artifact stays metadata-only, includes catalog-depth, refresh-safety, surface-depth, traceability, blocker-priority, execution-lock, visible detail, visible next action, current/expected catalog-fingerprint match state, and local record-validation evidence, and returns to review when its six-surface catalog fingerprint or attached approval/audit/rollback/permission validation no longer matches the current provider state.
   ...
   - Confirm Owner Testing validation rows, Connection Dialog catalog smoke rows, Provider Readiness catalog-depth rows, Refresh Safety depth rows, Surface Depth rows, Provider Traceability rows, local approval/audit/rollback/permission record validation, and Provider Blocker Priority rows report the same six provider surfaces as one linked metadata-only safety chain, with execution still visible as a separate lock after approval, audit, rollback, and permission are recorded.
...
11. `Permissions`
    ...
    - Confirm the Phase 8 risk blocker-priority queue ranks the exact top permission, approval, evidence, audit, rollback, disabled-path, or traceability blocker, shows whether audit review can address it, and keeps remaining-goal or PM traceability blockers out of audit-review actions before mutation paths grow.
