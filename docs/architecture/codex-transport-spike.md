...
For repeatable local validation, `npm.cmd run smoke:phase3` runs the ignored Tauri smoke tests for live-control, active-turn interrupt, and active-turn steer. This command is opt-in because it can send tiny read-only prompts through the local runtime; normal `npm test` and `cargo test` do not run these live smoke checks. `npm.cmd run smoke:phase3:record` runs the same smoke harness and writes `local_private/phase3-command-validation-record.json`, which Owner Testing can load from desktop mode or import as a CLI validation record without treating it as persisted desktop UI proof, plus `local_private/phase3-smoke-proof-bundle.json`, which Owner Testing can load from desktop mode or import through the desktop smoke readiness rows and still filters to desktop-executed proof rows. The direct `Load recorded` path is desktop-gated because browser preview cannot read workspace-local private artifacts through Tauri.
