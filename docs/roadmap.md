...
| Phase 7: Platform Capabilities | Planned | Slash commands now expose refreshable catalog snapshots and structured command feedback; plugins, automations, MCP, personalization, permissions, and audit state become live provider-backed surfaces. |
...
- Refreshable seeded option catalog so default plugins, skills, MCP servers, and commands can stay aligned with the connected provider.
