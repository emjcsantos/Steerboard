# Arena Operating Modes

Steerboard's Arena is the primary planning, chat, and monitoring surface. The same panel system must support focused work, orchestrated multi-agent work, and independent project monitoring.
...
- The Arena can use `1x1`, `1x2`, or `2x1`.
...
Use Steerboard as a command Arena for one orchestrated run.
...
- The Arena can scale from `2x1` to `3x3` depending on worker count.

## Mode 3: Independent Project Monitor

Use Steerboard to watch several unrelated projects or runs at the same time.

- Each Arena panel can bind to a different project, run, or session.
...
## Mode 4: Adaptive Arena Canvas

Use Steerboard as a freeform Arena when fixed presets are too rigid.

- Users can add panels from chats, projects, runs, tasks, evidence views, and provider tool surfaces.
- Users can drag a chat or whole project from the left rail into the Arena center.
...
- The Arena layout selector must be a dropdown or combobox, not a long row of preset buttons.
...
- The Arena toolbar should compress layout, focus, panel activity, hidden queue, and monitor controls into one row.
...
- Slash commands and integration controls are scoped to the focused panel unless the user explicitly applies them to a run, project, or Arena group.

## Optional Project Management Lane Relationship

The project management lane is optional. It can deploy structured work into any Arena mode when users want stronger pipeline visibility, tracking, readiness review, or change management:

- a single focused run,
- one orchestrator with worker panels,
- or several independent project panels.

The deploy preview must show which mode will open and which panels will be created, but users should still be able to plan directly in the Arena chat without using this lane.
