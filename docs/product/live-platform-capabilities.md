...
| Slash Command Registry | 39% | Provider-neutral command catalog, command state model, panel-scoped composer suggestions, preview routing, unsupported/unavailable blocking, visible provider-route transcript evidence, desktop-backed safe provider capability refresh, and owner validation for current metadata/status snapshots are in place; arbitrary command/tool execution remains pending. |
| Skills Catalog | 21% | Provider-neutral skill model, dialog surface, provider-only row support, desktop-backed safe provider metadata refresh, and owner validation for current metadata/status snapshots are in place; skill execution remains pending. |
| MCP Manager | 21% | Provider-neutral MCP model, dialog surface, provider-only row support, desktop-backed safe provider metadata refresh, and owner validation for current metadata/status snapshots are in place; MCP tool execution remains pending. |
| Plugin Manager | 21% | Provider-neutral plugin model, dialog surface, provider-only row support, desktop-backed safe provider metadata refresh, and owner validation for current metadata/status snapshots are in place; plugin execution remains pending. |
| Automations Manager | 21% | Provider-neutral automation model, dialog surface, provider-only row support, desktop-backed safe provider metadata refresh, and owner validation for current metadata/status snapshots are in place; scheduling and execution remain pending. |
| Personalization Center | 21% | Provider-neutral personalization model, settings entry, provider-only row support, desktop-backed safe provider metadata refresh, and owner validation for current metadata/status snapshots are in place; profile mutation remains pending. |
