...
| Live Panel Chat | 46% | Visible desktop panels can send through the runtime adapter, render normalized assistant/status/error output, and expose compact interrupt/retry/steer controls; browser preview remains local fallback. Panel-session persistence, duplicate identity guards, stream isolation foundations, native two-panel smoke, control-readiness smoke, and explicit active-turn interrupt smoke proof paths are in place; desktop owner execution of those smokes, active-turn steer smoke, and incremental UI streaming remain pending. |
