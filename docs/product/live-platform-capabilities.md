...
| Slash Command Registry | 35% | Provider-neutral command catalog, command state model, panel-scoped composer suggestions, preview routing, unsupported/unavailable blocking, and desktop-backed safe provider capability refresh are in place; live command execution remains pending. |
