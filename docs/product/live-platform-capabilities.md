...
| Live Panel Chat | 44% | Visible desktop panels can send through the runtime adapter, render normalized assistant/status/error output, and expose compact interrupt/retry/steer controls; browser preview remains local fallback. Panel-session persistence, duplicate identity guards, stream isolation foundations, native two-panel smoke, and control-readiness smoke proof paths are in place; desktop owner execution of those smokes, active-turn interrupt/steer smoke, and incremental UI streaming remain pending. |
