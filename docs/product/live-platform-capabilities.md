...
| Live Panel Chat | 50% | Visible desktop panels can send through the runtime adapter, render normalized assistant/status/error output, expose compact interrupt/retry/steer controls, and show unsupported-control plus owner-testing control-readiness evidence for unavailable lifecycle actions; browser preview remains local fallback. Panel-session persistence, duplicate identity guards, stream isolation foundations, native two-panel smoke, control-readiness smoke, explicit active-turn interrupt and steer smoke proof paths, and a diagnostic Phase 3 exit-gate evidence rollup are in place; desktop owner execution of those smokes, incremental UI streaming, and fork/resume/archive support remain pending. |
