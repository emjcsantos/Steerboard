...
| Live Panel Chat | 43% | Visible desktop panels can send through the runtime adapter, render normalized assistant/status/error output, and expose compact interrupt/retry/steer controls; browser preview remains local fallback. Panel-session persistence, duplicate identity guards, stream isolation foundations, and a native two-panel smoke proof path are in place; desktop owner execution of that smoke, live control smoke, and incremental UI streaming remain pending. |
