...
| Slash Command Registry | 40% | Provider-neutral command catalog, command state model, panel-scoped composer suggestions, preview routing, unsupported/unavailable blocking, visible provider-route transcript evidence, desktop-backed safe provider capability refresh, owner validation, and all-catalog provider refresh smoke proofing for current metadata/status snapshots are in place; arbitrary command/tool execution remains pending. |
