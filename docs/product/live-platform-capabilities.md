...
| Slash Command Registry | 38% | Provider-neutral command catalog, command state model, panel-scoped composer suggestions, preview routing, unsupported/unavailable blocking, visible provider-route transcript evidence, and desktop-backed safe provider capability refresh are in place; arbitrary command/tool execution remains pending. |
