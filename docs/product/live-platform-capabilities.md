...
| Live Panel Chat | 49% | Visible desktop panels can send through the runtime adapter, render normalized assistant/status/error output, expose compact interrupt/retry/steer controls, and show unsupported-control evidence for unavailable lifecycle actions; browser preview remains local fallback. Panel-session persistence, duplicate identity guards, stream isolation foundations, native two-panel smoke, control-readiness smoke, and explicit active-turn interrupt and steer smoke proof paths are in place; desktop owner execution of those smokes, incremental UI streaming, and fork/resume/archive support remain pending. |
| Slash Command Registry | 41% | Provider-neutral command catalog, command state model, panel-scoped composer suggestions, preview routing, unsupported/unavailable blocking, visible provider-route transcript evidence, evidence-driven owner testing readiness, desktop-backed safe provider capability refresh, owner validation, and all-catalog provider refresh smoke proofing for current metadata/status snapshots are in place; arbitrary command/tool execution remains pending. |
| Skills Catalog | 22% | Provider-neutral skill model, dialog surface, provider-only row support, desktop-backed safe provider metadata refresh, owner validation, and all-catalog provider refresh smoke proofing for current metadata/status snapshots are in place; skill execution remains pending. |
| MCP Manager | 22% | Provider-neutral MCP model, dialog surface, provider-only row support, desktop-backed safe provider metadata refresh, owner validation, and all-catalog provider refresh smoke proofing for current metadata/status snapshots are in place; MCP tool execution remains pending. |
| Plugin Manager | 22% | Provider-neutral plugin model, dialog surface, provider-only row support, desktop-backed safe provider metadata refresh, owner validation, and all-catalog provider refresh smoke proofing for current metadata/status snapshots are in place; plugin execution remains pending. |
| Automations Manager | 22% | Provider-neutral automation model, dialog surface, provider-only row support, desktop-backed safe provider metadata refresh, owner validation, and all-catalog provider refresh smoke proofing for current metadata/status snapshots are in place; scheduling and execution remain pending. |
