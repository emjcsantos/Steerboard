...
- Phase 3 clearance package, blocker-priority queue, traceability rows, command plan, and handoff gate with exact open blockers, stale-proof review, command-addressable blocker status, active-goal and PM child linkage, PM/evidence-key proof links, separate live-control/interrupt/steer command-plan rows, next smoke action, local owner handoff record/clear actions, current-evidence fingerprint matching, and provider-integration boundary.
