...
- Optional project/program planning lane with local dispatch previews, role-panel plan scaffolds, saved dispatch review records, and Phase 7 review-depth checks for role coverage, attempts, handoff tasks, validation gates, and the live-worker lock.
...
| Planning and dispatch loop | Next | Convert staged project/program work into orchestrator, implementer, validator, and integration panels with scoped handoff packets, saved review records, visible attempt limits, validation gates, and no-live-worker review-depth checks. |
...
| Planning and dispatch loop | Phase 7 | Next | Turn staged PM work into orchestrator, implementer, validator, and integration handoff packets with visible attempt limits, dispatch review depth, validation gates, and live-worker locks. | Use dispatch review records and review-depth checks to audit role counts, attempt limits, handoff tasks, validation gates, and live-worker execution locks before any live worker session spawning. |
