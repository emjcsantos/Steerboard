...
Overall completion: 41%. Next milestone: Platform capabilities. Next step: Extend the same safe provider refresh pattern to plugins, MCP, automations, and personalization, then run the owner testing checklist for command and skill refresh behavior while keeping arbitrary terminal commands, Git mutation, MCP/plugin/automation execution, runtime/profile mutation, and external-service actions disabled.
...
| Platform capabilities | In progress | Panel slash commands and Skills catalog refresh through desktop-backed safe provider capability bridges with ready, preview, blocked, unsupported, and unavailable feedback while mutation-class platform actions remain non-executable. |
...
| Cockpit monitor and operating modes | Complete core cockpit panels, control affordances, and visible operating-mode cues. | In progress | 62% | Multi-panel cockpit, adaptive magnetic layout with project stack drag-in, session drag-in, drop previews, keyboard adjustment, and adaptive project-drop template selection exist; local chat-lane scaffolding includes panel-keyed live session chat and per-panel session controls, with native two-panel smoke still pending. | Run native two-panel smoke and live-control smoke, then continue command capability and live-session hardening work. |
...
| Platform capabilities | Make commands, plugins, automations, MCP, personalization, permissions, and audit state live. | In progress | 40% | Panel slash commands and Skills catalog refresh through desktop-backed safe provider capability bridges with ready, preview, blocked, unsupported, and unavailable feedback while mutation-class platform actions remain non-executable. | Extend the same safe provider refresh pattern to plugins, MCP, automations, and personalization, then run the owner testing checklist for command and skill refresh behavior while keeping arbitrary terminal commands, Git mutation, MCP/plugin/automation execution, runtime/profile mutation, and external-service actions disabled. |
...
| Owner testing hardening | Make live-functionality checks repeatable through a local checklist, failure fixtures, and docs consistency checks. | In progress | 34% | A provider-neutral checklist and fixture layer now verify Phase 10B runner contract behavior for approved requests, read-only terminal probe constraints, disabled mutation paths, and redacted audit output. | Run the full local testing checklist, verify approval-to-probe behavior, provider command and skill refresh behavior, and disabled-path evidence (terminal, Git, MCP, plugin, automation, runtime, profile, and external), and keep desktop-backed mutation as a future phase. |
