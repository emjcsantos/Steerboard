...
| Cockpit monitor and operating modes | Complete core cockpit panels, control affordances, and visible operating-mode cues. | 60% | Core cockpit visibility is in place with active attention flows, monitor status wiring, and toolbar focus/clear controls. | Finalize monitoring depth and keep toolbar focus/clear behavior consistent before hardening operating-mode handoffs. |
...
| Packaging and installation | Prepare install, environment, and distribution path for dependable rollout. | 15% | Basic packaging decisions are started and setup planning is underway. | Finalize release packaging and installation scripts for repeatable onboarding. |
| Optional project management lane | Define the optional lane scope and keep it intentionally dormant unless priority changes. | 0% | Paused: optional lane is deferred while core delivery milestones advance. | Remain paused; reassess only after critical milestones move to a stable operating state. |
